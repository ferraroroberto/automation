...
- **Verification before shipping:** `powershell -File scripts\verify-before-ship.ps1` — byte-compiles every module and runs the four unit-test suites that do exist (`system/foldersearcher`, `system/test_local_config_hygiene.py`, `system/wifi`, `video`). It must exit 0.
