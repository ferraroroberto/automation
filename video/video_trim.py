...
def trim_video(input_path, start_sec, end_sec, progress_callback=None, status_callback=None):
    """
    Trim video between start_sec and end_sec (float seconds).
    Optional progress_callback(percent), status_callback(message).
...
class VideoTrimApp:
    def __init__(self):
        ...
        self.queue = []  # List of (input_path, start_sec, end_sec)
    ...
    def _build_ui(self):
        ...
        # Time controls
        time_frame = tk.LabelFrame(self.root, text="Trim range (mm:ss.s or +/- 0.1s)", padx=10, pady=8)
        time_frame.pack(fill=tk.X, padx=10, pady=5)
        time_frame.columnconfigure(2, weight=1)

        tk.Label(time_frame, text="Start:").grid(row=0, column=0, sticky="w", pady=2)
        ...
        tk.Button(time_frame, text="+0.1", width=5, command=self._start_add).grid(row=0, column=2, padx=2, pady=2)
        tk.Button(time_frame, text="-0.1", width=5, command=self._start_sub).grid(row=0, column=3, padx=2, pady=2)

        tk.Label(time_frame, text="End:").grid(row=1, column=0, sticky="w", pady=2)
        ...
        tk.Button(time_frame, text="+0.1", width=5, command=self._end_add).grid(row=1, column=2, padx=2, pady=2)
        tk.Button(time_frame, text="-0.1", width=5, command=self._end_sub).grid(row=1, column=3, padx=2, pady=2)
    ...
    def _add_to_queue(self):
        ...
        if self.duration_sec and (start < 0 or end > self.duration_sec):
            ...
            return
        self.queue.append((path, start, end))
        name = os.path.basename(path)
        self.queue_listbox.insert(tk.END, f"{name}  {seconds_to_time_str(start)} → {seconds_to_time_str(end)}")
        self.status_var.set(f"Added. Queue: {len(self.queue)} trim(s)")
    ...
    def _process_queue(self):
        ...
        def worker():
            ...
            for i, (path, start, end) in enumerate(self.queue, 1):
                def status_cb(msg):
                    self.root.after(0, lambda m=msg: self.status_var.set(m))
                def progress_cb(pct):
                    # Scale by job index for overall progress
                    overall = (i - 1) / total * 100 + (pct / 100) / total * 100
                    self.root.after(0, lambda o=overall: self._update_progress(o))
                self.root.after(0, lambda: self.status_var.set(f"Processing {i}/{total}: {os.path.basename(path)}"))
                try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Video Trimmer - trim videos with FFmpeg")
    ...
    if args.gui or (not args.file and not args.start and not args.end):
        ...
        if not args.start:
            args.start = ask_time("Enter START time")
        if not args.end:
            args.end = ask_time("Enter END time")
